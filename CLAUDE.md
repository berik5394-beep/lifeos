...
### ВАЖНО: мягкая механика
- Питомец НЕ умирает. Максимум — болеет и лежит. Это мотивация, не стресс.
- Пропуск одного приёма пищи — ничего страшного, питомец просто "думает о еде"
- Целый день без еды — питомец заболел (мягко, не драматично)
- Можно "покормить" питомца вручную (кнопка) — но лучше через реальные привычки
- Если пользователь возвращается после перерыва — питомец радуется: "Ты вернулся!"
...
model Pet {
  ...
  costume     String?  // "crown", "superhero", "party_hat" — разблокируются за достижения
  createdAt   DateTime @default(now())
