...
export const LOCAL_TOOLS = [
  ...
  {
    // @deprecated 9A.6 — реализация в tools/get-trip.ts.
    // Схема тут до 9A.8 (свич claude-agent на anthropicSchemas).
    name: 'get_trip',
    description:
  ...
  {
    // @deprecated 9A.7 — реализация в tools/get-goal-progress.ts.
    // Схема тут до 9A.8 (свич claude-agent на anthropicSchemas).
...
export async function runLocalTool(
  ...
  try {
    switch (name) {
      ...
      case 'get_trip': {
        // 9A.6: мигрировано в реестр (tools/get-trip.ts).
        // Свич схем claude-agent на SSOT — 9A.8.
        console.warn(
          `[deprecated 9A] runLocalTool.get_trip → registry (user=${userId})`,
        );
        const r = await runRegistryTool('get_trip', input, { userId });
        return JSON.stringify(r);
      }
      case 'get_weather': {
      ...
      case 'get_goal_progress': {
        // 9A.7: мигрировано в реестр (tools/get-goal-progress.ts),
        // вся Phase 2.5 логика там 1:1. Свич claude-agent — 9A.8.
        console.warn(
          `[deprecated 9A] runLocalTool.get_goal_progress → registry (user=${userId})`,
        );
        return JSON.stringify(
          await runRegistryTool('get_goal_progress', input, { userId }),
        );
      }
      default:
