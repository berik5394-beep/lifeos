...
export function deterministicIntent(text: string): VoiceIntent | null {
  ...
  // SSOT Step 6: финансовый regex УДАЛЁН намеренно. Раньше узкие
  // паттерны ловили лишь часть фраз, а свободные («40000 — комиссия
  // Kaspi») проваливались мимо → чат-агент выдумывал «записал»
  // (корень бага #1). Теперь add_expense/add_income классифицирует
  // Claude parseIntent (есть в схеме ниже), исполняет аудируемый
  // реестр с needsConfirm:true. Детерминированного money-пути нет.

  // complete_habit: "отметь привычку бег", "отметь бег", "выполнил чтение",
  // "сделал зарядку" (одна привычка)
  let m = text.match(/^(?:отметь|выполнил[аи]?|сделал[аи]?|закрой)\s+(?:привычку\s+)?(.+)$/i);
  if (m) {
    const rest = m[1].trim();
