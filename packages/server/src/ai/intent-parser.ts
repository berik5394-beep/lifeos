...
const TRAVEL_NOUNS = /(рейс|билет(ы|а)?|самол[её]т|перел[её]т|пол[её]т|такси|машин[ауы]?|отел[ьья]|гостиниц[ауы]?|номер в|поездк[ауи]|путешестви|командировк|отпуск|тур\b|путёвк[ауи]?|виз[ауы])/i;
...
function deterministicIntent(text: string): VoiceIntent | null {
  ...
  const STRONG_TRAVEL =
    /(слетать|слетаю|полететь|полечу|полетим|поехать|поедем|съездить|съезжу|еду в|едем в|лечу в|хочу слетать|хочу поехать|хочу съездить|надо слетать|надо съездить|собираюсь в|планирую слетать|планирую поехать|планирую съездить|планирую поездку|спланируй поездку)\b/i;
